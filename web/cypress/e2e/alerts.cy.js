describe('Validações de Alertas em JavaScript', () => {
    ...
    it('Deve vallidar a mensagem de alerta', () => {

        //window:alert Antes da Ação de Clicar e visualizar
    ...
    it('Deve intragir com um prompt. inserir um texto e validar uma mensagem', () => {
        
        //simulando comportamento do prompt box
        cy.window().then((win) => {
            cy.stub(win, 'prompt').returns('Blando Alexandre')
        })

        cy.on('window:alert', (msg) => {
            expect(msg).to.equal('Olá Blando Alexandre! Boas-vindas ao WebDojo!')
        })
